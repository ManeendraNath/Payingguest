...
//================= get the meeting  details ====================
        $scope.getMailView = function (mId) {
...
//=================== maker url and get Data =============================================
            $("#modalDSho").html("Please Wait ...");
            var usrUrl = ALL_BS_URL + "?pos=getSmaildetail&hdata=hide&mid=" + mId + "&bkclk=90";
...
//=============================== action pointer ==================================
        $scope.actionOnCrItem = function (mId, relId, utype) {



            var usrUrl = ALL_BS_URL + "?pos=getActionFormMail&hdata=hide&mid=" + mId + "&relId=" + relId + "&utype=" + utype + "&bkclk=90";

            giveServiceToAll.getMViewDetail(mId, usrUrl, relId, utype).then(function (resolve) {
...
//========================== close the action Button ===============================
        $scope.callDecisionComment = function (refId, relId, utype) {

            giveServiceToAll.UpdateCommentOnDecision(refId, relId, utype);
...
//--------------- =========== writeInEmailDoc ================
        $scope.writeInEmailDoc = function () {

            var docRed = $(document).find("#docSub");
            docVal = docRed.val();
            docVal = docVal.trim();
            if (docVal.length > 0)
            {

                var emailRed = $(document).find("#emailSub");
...
//======================= ------------- all form Action will Done Here --------------------------------
            var fValid = "Ok";
            var mcatRef = $(document).find("#selectCat");
            mcatVal = mcatRef.val();
            ...
            if (mcatVal == "No")
            {
                mess = "Please Select The Category, Fill All Required Fields";
                this.errorMess(mess);
                alert(mess);
                return false;
            ...
            //---------------------- select Doc Type -----------------------------                                      
            var mdocRef = $(document).find("#selectDocType");
            mdocVal = mdocRef.val();
            ...
            if (mdocVal == "No")
            {
                mess = "Please Select The Document Type, Fill All Required Fields";
                this.errorMess(mess);
                alert(mess);
                return false;
...
//================================================ validation box =====================================
            var dlDateRef = $(document).find('#dLDte');
            dlDateVal = dlDateRef.val();
            fValid = this.fileDvalid(dlDateVal);
            if (dlDateVal.length < 4)
            {

                mess = "Document/Letter Date Required ,Please Fill Required Fields";
                this.errorMess(mess);
                alert(mess);
                //this.errorMess();
                return false;
...
//---------------------------- Expired Date --------------------------------
            /*/ var dEDteRef=$(document).find('#dEDte');
             dEDteVal=dEDteRef.val();
             fValid=this.fileDvalid( dEDteVal);
             if(dEDteVal.length<4)
             {
             
             mess="Document Expiry Date Required ,Please Fill Required Fields";
             this.errorMess(mess);
             alert( mess);
             //this.errorMess();
             return false;
...
//================================================ validation box =====================================
            ...
            //----------------------- subject docSub------------------------------------
            var docSubRef = $(document).find("#docSub");
            docSubVal = docSubRef.val();
            var emailSubRef = $(document).find("#emailSub");
            emailSubVal = emailSubRef.val();
...
//========================= captcha varification =================================
            ...
            if (getCptVal.length != 6)
            {
                ...
                mess = "Invalid Captcha Code, Please Enter Similar Code";
                this.errorMess(mess);
                alert(mess);
                //this.errorMess();
                return false;
...
//----------------- CKEDITOR VALUE  FOR  MESSAGE -----------------------------
            ...
            var urlVal = ALL_BS_URL + "/mailercreator";
            /*=========== attendee List ===============
            ...
            //   console.log(JSON.stringify(newAttendeeLst));

            // return false;
            $getMSDetail = {"mcat": mcatVal, "mDoctype": mdocVal, "lDate": dlDateVal, "fno": getFNumberVal, "dExpDate": dEDteVal, "docSub": docSubVal, "emailSub": emailSubVal, "emailBody": getMeetDescript};

            //console.log(JSON.stringify($getMSDetail));

            // allDataValue=getFNumberVal+"==="+ getMDateVal+"==="+getchairVal+"===="+ getMeetDescript +"===="+getmomVal;
            var allDataValues = {"url": urlVal, "mailBdy": $getMSDetail, "cpCode": getCptVal, "nwAttendee": newAttendeeLst};
...
//====================================================/*/
                ...
                if (reData['status'] != undefined)
                {
                    ...
                    {
                        alert(" Mail  created Successfully");
                        getReUrl = reData['data']['url'];
                        //location.assign(getReUrl);
                        returnurl = 'https://dbtbharat.gov.in/dbtsamiksha?pos=viewSendMailitems&mStatus=MailSend&actZone=OM';
...
//==================== created form data  is submitted here ============================================
        ...
        this.formWaiterLoad = function () {
            ...
            getSubButtOnId.innerHTML = "<img src='/images/animated.gif' width='20'/>Please Wait..";
...
//---------------------- loader with id ---------------------------
        this.formWaiterLoadId = function (refId) {
            ...
            getSubButtOnId.innerHTML = "<img src='/images/animated.gif' width='20'/> Wait..";
...
//================================ add the action of the Decision making body =========================      
        this.addWorkDecisionBody = function () {
            ...
            addActionAssigner = '<button type="button" class="btn btn-default" id="attendcr12" data-tole="modal" data-target="#bactModal">Add Attendees </button>';
            rowCPoint++;
            ...
            rowHolder += '<div class ="col-sm-4">' + addActionAssigner + '</div>';
...
//========================== get the value item for Show ====================
        this.getMViewDetail = function (mId, usrUrl, relId, utype) {
            ...
            var mk_data = {url: re_url, usr: mId, relId: relId, utype: utype};
            // console.log(re_url);
...
//====================================== update columen ==================
        this.UpdateCommentOnDecision = function (refId, relId, utype) {
...
//====================================== ajax request ===================================	  
            ...
            var re_url = ALL_BS_URL + "/commentactmail";




            var mk_data = {url: re_url, comment: comRefVal, idref: getMRef, relId: relId, utype: utype, mailId: refId, docId: docRef};
            ...
            // return false;
            bntIdRef = "descomm" + refId;
            this.formWaiterLoadId(bntIdRef);
            ...
            allFMethods.commentwithDoc(mk_data).success(function (reData) {
                ...
                getObjRefId.reopenFrmActId(bntIdRef);
                //console.log(JSON.stringify(reData)); 
                //  alert(reData);
...
//---------------- make search on the value ----------------------
        this.mkSearchOnRequest = function (mtype, txtId, sdateP, endDateP) {
            ...
            if (chkOk == "Ok")
            {
                var usrUrl = ALL_BS_URL + "/insearch";
                var deferred = $q.defer();
                ...
                allFMethods.onlyContentValpOST(mk_data).success(function (reData) {
                    ...
                    if (reData['Status'] == "Success")
                    {
                        ...
                        angular.forEach(getDataObj, function (value, key) {
                            ...
                            mId = indiObj.mId;
                            docSub = indiObj.docSub;
                            lDate = indiObj.lDate;
                            mailCat = indiObj.mailCat;

                            mkdata += "<tr>";
                            mkdata += "<td>" + countP + "</td>";
                            mkdata += "<td>" + fileNo + "</td>";
                            mkdata += "<td>" + mailCat + "</td>";
                            mkdata += "<td>" + docSub + "</td>";
                            mkdata += "<td>" + lDate + "</td>";
                            mkdata += '<td><button class="btn btn-default" data-toggle="modal" data-target="#myModal" ng-click="getMailView(\'' + mId + '\')">View</button></td>';
