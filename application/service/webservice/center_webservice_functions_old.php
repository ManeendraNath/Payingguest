...
//	get_plain_ws_data
function get_plain_ws_data($url_details,$scheme_code,$statecode,$requestdate){
	$txt='';
	$responseData = '';
	$errormsg = '';
	...
	// soap service
		...
		//	$userpasswordplace = isset($url_details->userpassword_place) ? $url_details->userpassword_place : "";	// add new field in database	1 : in url, 2 : inside curl
			...
			if(!empty($tokenurl))	//	token is creating in run time (dynamic).
			{
				...
				$tokenkey=dynamic_token($tokenurl, $username, $password);	//	dynamic token creation
			}
		...
		// this condition must be managing from database
			if(strpos($url, 'onlinesales.licindia.in') !== false || strpos($url, '115.111.143.40') !== false || strpos($url, '10.248.107.200') !== false) {
		//	if($userpasswordplace==2) {	this will be correct condition after adding field in database and changing in new webservice entry
				foreach($parameter AS $key=>$value)
				{
